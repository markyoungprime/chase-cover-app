...
if "sketch_created" not in st.session_state:
    ...
    st.session_state.adjusted_width = 0.0
    st.session_state.adjusted_length = 0.0
    st.session_state.width = 0.0
    st.session_state.length = 0.0
    st.session_state.flange_length = 0.0
    st.session_state.add_kickout = True
...
with st.form(key="measurement_form"):
    ...
    with tab1:
        ...
        with col4:
            add_kickout = st.checkbox("Add Kickout?", value=st.session_state.add_kickout, 
        ...
        with st.expander("Chimney Holes", expanded=True):
            ...
            for i in range(num_holes):
                ...
                if ("left" in distances and "right" in distances) and ("front" in distances and "back" in distances):
                    if abs(diameter_x - diameter_y) > 0.1:
...
if submit_button:
    if not all_valid:
        ...
        st.session_state.flange_length = flange_length
        st.session_state.add_kickout = add_kickout
        st.session_state.adjusted_width = width + fit_tolerance
        st.session_state.adjusted_length = length + fit_tolerance
        
        fig2d, ax2d = plt.subplots()
        ax2d.add_patch(plt.Rectangle((0, 0), st.session_state.adjusted_width, st.session_state.adjusted_length, fill=False, edgecolor="black", linestyle="-"))
        ax2d.add_patch(plt.Rectangle((-flange_length, -flange_length), st.session_state.adjusted_width + 2 * flange_length, 
                                     st.session_state.adjusted_length + 2 * flange_length, fill=False, edgecolor="blue", linestyle="-"))
        if add_kickout:
            ax2d.add_patch(plt.Rectangle((-flange_length - 0.5, -flange_length - 0.5), 
                                         st.session_state.adjusted_width + 2 * flange_length + 1, st.session_state.adjusted_length + 2 * flange_length + 1, 
                                         fill=False, edgecolor="black", linestyle="-"))
            ax2d.add_patch(plt.Rectangle((-flange_length - 0.875, -flange_length - 0.875), 
                                         st.session_state.adjusted_width + 2 * flange_length + 1.75, st.session_state.adjusted_length + 2 * flange_length + 1.75, 
                                         fill=False, edgecolor="gray", linestyle="-"))
        else:
            ax2d.add_patch(plt.Rectangle((-flange_length - 0.375, -flange_length - 0.375), 
                                         st.session_state.adjusted_width + 2 * flange_length + 0.75, st.session_state.adjusted_length + 2 * flange_length + 0.75, 
                                         fill=False, edgecolor="gray", linestyle="-"))
        ...
        ax2d.text(st.session_state.adjusted_width/2, -flange_length - 1, f"Width = {st.session_state.adjusted_width:.2f}", ha="center", va="top", fontsize=10)
        ax2d.text(-flange_length - 1, st.session_state.adjusted_length/2, f"Length = {st.session_state.adjusted_length:.2f}", ha="right", va="center", fontsize=10, rotation=90)
        ax2d.text(st.session_state.adjusted_width + flange_length + 1, st.session_state.adjusted_length/2, f"Flange = {flange_length:.2f}", ha="left", va="center", fontsize=8)
        
        ax2d.set_xlim(-flange_length - 2, st.session_state.adjusted_width + flange_length + 2)
        ax2d.set_ylim(-flange_length - 2, st.session_state.adjusted_length + flange_length + 2)
...
if st.session_state.sketch_created and st.session_state.jpg_buffer:
    ...
    if send_button and st.session_state.uploaded_files_data:
        if not st.session_state.sketch_created:
            st.error("Please create a sketch before sending to shop.")
        else:
            try:
                ...
                if not sender_email or not sender_password:
                    raise ValueError("Email credentials not set in environment variables. Please set SENDER_EMAIL and SENDER_PASSWORD in your .env file or Streamlit Cloud secrets.")
                
                dxf_buffer = create_dxf_buffer(st.session_state.adjusted_width, st.session_state.adjusted_length, 
                                              st.session_state.flange_length, st.session_state.add_kickout, 
                                              st.session_state.holes, st.session_state.project_name)
                
                total_width = st.session_state.adjusted_width + 2 * st.session_state.flange_length + (1.75 if st.session_state.add_kickout else 0.75)
                total_length = st.session_state.adjusted_length + 2 * st.session_state.flange_length + (1.75 if st.session_state.add_kickout else 0.75)
                ...
                body += f"Length (Front to Back): {st.session_state.length:.2f} inches (Adjusted: {st.session_state.adjusted_length:.2f} inches with {st.session_state.fit_tolerance:.2f}\" tolerance)\n"
                body += f"Width (Left to Right): {st.session_state.width:.2f} inches (Adjusted: {st.session_state.adjusted_width:.2f} inches with {st.session_state.fit_tolerance:.2f}\" tolerance)\n"
                if total_length >= 48 or total_width >= 48:
                    body += "**REQUIRES MORE THAN 1 SHEET TO FABRICATE**\n"
                body += f"Outer flange length (turndown): {st.session_state.flange_length:.2f} inches\n"
                body += f"Add Kickout: {st.session_state.add_kickout}\n"
                ...
                data_to_save = {
                    ...
                    "fit_tolerance": st.session_state.fit_tolerance,
                    "adjusted_width": st.session_state.adjusted_width,
                    "adjusted_length": st.session_state.adjusted_length,
                    "flange_length": st.session_state.flange_length,
                    "add_kickout": st.session_state.add_kickout,
...
5. Click 'Create Sketch' to see the 2D sketch.
6. Use 'Save Sketch to Device' to save the JPG or 'Send to Shop' to email data, photos, and DXF with JSON autosave.
""")